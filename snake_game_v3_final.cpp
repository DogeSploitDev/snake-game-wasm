...
private:
    ...
    void resetGame() {
        ...
        // Spawn snake in center of screen, moving right
    ...
    void renderUI() {
        ...
        if (gameOver) {
            ...
            if (inputActive) {
                ...
                // Show high scores first, then restart instruction below them
                renderHighScores();
                drawTextCentered("Press R to Restart", windowWidth / 2, windowHeight - 50, {255, 255, 255});
            }
            return;
        }
    ...
    void renderHighScores() {
        // Position high scores in the middle-lower section, leaving room for restart text at bottom
        int startY = windowHeight / 2 - 20;
        drawTextCentered("High Scores:", windowWidth / 2, startY, {255, 255, 0});
        for (int i = 0; i < maxHighScores; ++i) {
            std::stringstream ss;
            ss << (i + 1) << ". " << highScores[i].username << " - " << highScores[i].score;
            drawTextCentered(ss.str(), windowWidth / 2, startY + 30 + i * 25, {255, 255, 255});
        }
    }
